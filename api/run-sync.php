...
$isCliRuntime = (PHP_SAPI === 'cli');
if (!$isCliRuntime && !headers_sent()) {
...
if (!$ffmpeg->isAvailable()) {
    sendProgress('ffmpeg', 'warning', 'FFmpeg missing, attempting runtime bootstrap...', 16, $stats);
    try {
        $runtimeBootstrap = new RuntimeBootstrap($pdo);
        $runtimeInstall = $runtimeBootstrap->ensureFFmpegAvailable(true);
        if (!empty($runtimeInstall['success'])) {
            $ffmpeg = new FFmpegProcessor();
        }
    } catch (Throwable $runtimeError) {
        // Fall through to the standard error below with the refreshed probe result.
    }
}
if (!$ffmpeg->isAvailable()) {
