...
$resultPayload = isset($payload['payload']) && is_array($payload['payload']) ? $payload['payload'] : $payload;

$manager = new LocalAgentManager($pdo);
$result = $manager->completeJob($jobId, $claimToken, $resultPayload, (string)($_SERVER['REMOTE_ADDR'] ?? ''));
if (!$result['success']) {
