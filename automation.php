            ...
            <div class="card rounded-lg" data-automation-id="<?= $automation['id'] ?>" data-automation-name="<?= htmlspecialchars($automation['name']) ?>">
                <div class="p-4 flex items-center justify-between border-b border-gray-800">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <?php else: ?>
                            <button type="button" onclick="runAutomationLive('<?= $automation['id'] ?>')" class="inline-flex items-center gap-2 px-3 py-2 hover:bg-gray-700 rounded text-emerald-300 border border-emerald-500/20 bg-emerald-500/10" title="Run this automation locally right now">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                                <span class="text-xs font-medium">Run Local</span>
                            </button>
                            <button type="button" onclick="runAutomationGithub('<?= $automation['id'] ?>')" class="inline-flex items-center gap-2 px-3 py-2 hover:bg-gray-700 rounded text-sky-300 border border-sky-500/20 bg-sky-500/10" title="Dispatch this automation to GitHub runner">
                                ...
                                <span class="text-xs font-medium">Run GitHub</span>
                            </button>
                        <?php endif; ?>
                ...
                <div class="p-4">
                    ...
                    <div class="flex flex-wrap gap-2 mt-4">
                        <?php if (($automation['run_mode'] ?? 'local') === 'github_runner'): ?>
                            <span class="px-2 py-1 bg-sky-500/10 rounded text-xs text-sky-300 border border-sky-500/20">
                                GitHub Runner
                            </span>
                        <?php else: ?>
                            <span class="px-2 py-1 bg-emerald-500/10 rounded text-xs text-emerald-300 border border-emerald-500/20">
                                Local Runner
                            </span>
                        <?php endif; ?>
        ...
        <form method="POST" id="createForm" novalidate class="overflow-y-auto" style="max-height: calc(90vh - 140px);">
            ...
            <div id="form_basic" class="p-4 space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Ye scheduled aur cron runs ka default mode hai. Card par manual buttons se aap kabhi bhi Local ya GitHub dono mein run kar sakte hain.</p>
                </div>
        ...
        <form method="POST" id="editForm" novalidate class="overflow-y-auto" style="max-height: calc(90vh - 140px);">
            ...
            <div id="edit_form_basic" class="p-4 space-y-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Ye saved default hai for scheduled/cron runs. Manual `Run Local` aur `Run GitHub` buttons dono alag se card par available rahenge.</p>
                </div>
...
function runAutomationSmart(automationId, runMode) {
    if (runMode === 'github_runner') {
        runAutomationGithub(automationId, 'github_runner');
        return;
    }
...
function runAutomationGithub(automationId, runMode = 'github_runner') {
    ...
    const url = 'api/start-automation.php?id=' + encodeURIComponent(automationId) +
        '&mode_override=' + encodeURIComponent(runMode || 'github_runner');

    fetch(url, { cache: 'no-store' })
        .then(r => r.json())
        .then(data => {
            if (!data || !data.success) {
                throw new Error((data && data.error) ? data.error : 'GitHub dispatch failed');
            }

            const status = data.status || 'running';
            setCardStatus(automationId, status);
            if (progressLog) {
                addCardLog(progressLog, data.message || 'Dispatched to GitHub runner.', 'success');
                if (data.run_url) {
                    addCardLog(progressLog, data.run_url, 'info');
                }
            }

            if (typeof showToast === 'function') {
                showToast(data.message || 'GitHub runner dispatched');
            }
        ...
        .catch(err => {
            ...
            if (progressLog) {
                addCardLog(progressLog, err.message || 'GitHub dispatch failed', 'error');
            }
            if (typeof showToast === 'function') {
                showToast(err.message || 'GitHub dispatch failed', 'error');
            }
        });
}
